...
- **The GPU works but the environment is mid-migration.** Host driver 566.07 (CUDA 12.7 era) cannot run a cu130 torch build; a swap to cu126 was started and its outcome is unverified. Check `torch.cuda.is_available()` before assuming anything. GPU is an RTX 3050 Laptop, **compute capability 8.6**, 4 GB VRAM — that clears `mini-flash-attention`'s SM 80+ floor, but 4 GB is tight and `max_model_len` / `gpu_memory_utilization` need tuning.
