...
- **CUDA graphs are skipped when speculation is on, and this is currently the feature's blocker.** Graphs are captured for one query row per request and a round runs K+1. Measured 2026-08-04 (`experiments/spec_breakeven.py`): graphs are worth **7.9x** on plain decode here, so a speculative round costs 10-21x a graphed decode step and break-even is unreachable at any acceptance rate — a perfect draft still leaves it 2.4-5x slower. The fix is to capture graphs for the verify and propose passes, both of which are decode-shaped; do that **before** training a draft (Phase 1), which would otherwise be wasted effort. Full numbers and the plan are in `PROGRESS.md`.
